...
function sectionHtml(section: SignalEmailSection, digest?: DeepSeekDigest | null) {
  ...
  return `<section style="border-top:1px solid #d8d3c8;padding:22px 0">
    ...
    ${signals.map((signal) => {
      ...
      <p style="font-size:11px;line-height:1.5;color:#9a958b;margin:6px 0 0">原始标题：${escapeEmailHtml(signal.title)}</p>
    </article>`;
    }).join("")}
...
export function buildMarketSignalsEmail({
  ...
  const html = `<div style="margin:0;padding:32px 16px;background:#f3f0e9;color:#11110f;font-family:Arial,'Noto Sans SC',sans-serif">
    <main style="max-width:680px;margin:auto;background:#fff;padding:36px 34px;border:1px solid #ded9cf">
      ...
      <h1 style="font-family:Georgia,'Noto Serif SC',serif;font-size:30px;line-height:1.2;margin:12px 0;color:#11110f">${escapeEmailHtml(digest?.headlineZh || "近期公开动态摘要")}</h1>
      ...
      ${digest ? `<p style="font-size:11px;line-height:1.6;color:#9a958b;margin:20px 0 0">摘要由 ${escapeEmailHtml(digest.model)} 基于邮件中列出的标题、来源与日期整理；原始链接保留供核验。</p>` : ""}
