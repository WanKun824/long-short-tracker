      ...
      {data && (
        <>
          <section className={styles.statusline}>
            <span><i /> 数据库已连接</span>
            <span>最近检查：{formatDateTime(data.lastRefreshAt)}</span>
          ...
          <section className={styles.grid}>
            ...
            <article className={styles.panel}>
              <div className={styles.panelHead}>
                <div><span>DELIVERY HEALTH</span><h2>邮件投递状态</h2></div>
              </div>
          ...
          <section className={styles.tablePanel}>
            <div className={styles.panelHead}>
              <div><span>DATA HISTORY</span><h2>13F快照历史</h2></div>
              <b>最近 {data.dataHistory.snapshots.length} 条</b>
            </div>
            <div className={styles.tableScroll}>
              <table>
                <thead><tr><th>检查时间</th><th>机构</th><th>报告期</th><th>持仓数</th><th>变动摘要</th><th>申报编号</th></tr></thead>
                <tbody>
                  {data.dataHistory.snapshots.map((snapshot) => (
                    <tr key={`${snapshot.fundId}-${snapshot.accession}`}>
                      ...
                      <td>{snapshot.holdingCount}</td>
                      <td>{changeSummary(snapshot)}</td>
                      <td><code>{snapshot.accession}</code></td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <section className={styles.tablePanel}>
            <div className={styles.panelHead}>
              <div><span>ALERT HISTORY</span><h2>最近邮件记录</h2></div>
              <b>不显示订阅邮箱</b>
            </div>
